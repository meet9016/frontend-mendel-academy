  ...
  <div className="max-w-[1380px] mx-auto">
    ...
    {loading ? (
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {plans.map((plan) => (
          <PlanCard key={plan._id} plan={plan} onEnroll={() => onEnroll(plan._id)} />
        ))}
      </div>
    )}
  </div>
  ...
  >    
    ...
    <div className="flex flex-col justify-between h-full space-y-6 mt-2">
      <div className="space-y-2">
        ...
        <h3 className="text-2xl font-bold ff-font-bold text-center">{plan.plan_day} Days</h3>

        <div className="space-y-2 text-center">
          <p className="text-3xl ff-font-bold font-bold text-primary">INR {formatPrice(plan?.plan_pricing ?? "")}</p>
        </div>
      ...
      <CommonButton onClick={onEnroll} pyClass="py-3" pxClass="px-20" fontWeight={700} fontSize={14}>
        Enroll Now
      </CommonButton>
    </div>
  </div>
