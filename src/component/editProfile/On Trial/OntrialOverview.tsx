    ...
    <div className="space-y-6">
      ...
      <div className="w-full bg-black  border-t-4 border-primary text-white rounded-xl p-6 md:px-8 md:py-6 flex flex-col md:flex-row justify-between items-center gap-6 ">
        ...
        <div className="flex items-start md:items-center gap-5 w-full md:w-auto">
          ...
          <div className="flex-1 space-y-1">
            {/* Badge with Breathing Dot */}
            <div className="inline-flex items-center gap-2 bg-[#2d2d2d] px-2.5 py-1 rounded text-[11px] font-medium text-primary tracking-widest uppercase">
            ...
            <h2 className="text-[19px] md:text-xl font-bold tracking-tight">
              You're on day 2 of your free trial
            </h2>
            <p className="text-[13.5px] text-[#ffffffb3] max-w-xl ">
              Your trial converts to the Step 1 + Step 2 annual plan ($249/yr) on May 8. Upgrade now to lock in pricing, or cancel before then in Membership.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 w-full md:w-auto shrink-0">
          <button className="flex-1 md:flex-none flex ff-font items-center justify-center gap-2 px-6 py-2.5 border border-gray-600 rounded-lg text-[13px]  hover:bg-white/5 transition-all">
            <FiPlay size={14} className="fill-current" /> Go to app
          </button>
          <button className="flex-1 md:flex-none px-6 py-2.5 bg-primary  text-black rounded-lg text-[13px] font-medium ">
            Upgrade now
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
              ...
              <p className="text-[26px]  ff-font mt-2">{item.value}</p>
      ...
      <div className="bg-black text-white border-t-4 border-primary rounded-2xl p-8 flex flex-col md:flex-row justify-between items-center shadow-xl">
        <div className="text-center md:text-left">
          <p className="text-[11px] font-semibold ff-font-bold  text-primary uppercase tracking-[0.1em] mb-2">Contact us</p>
          <h3 className="text-[22px] ff-medium font-bold mb-2">
            Questions about your studies or account?
          </h3>
          <p className="text-sm text-gray-400 ff-font max-w-md">
            Our support team is here to help — whether it's a question about a course, a billing issue, or feedback. Email us at <span className="text-primary underline cursor-pointer">info@mendelacademy.com</span>
          </p>
        </div>

        <button className="mt-6 md:mt-0 flex ff-font-bold  items-center gap-2 bg-primary text-black px-8 py-3 rounded-md text-sm  ">
          <FiMail size={18} />
          Email support
        </button>
      </div>
...
export default OntrialOverview;