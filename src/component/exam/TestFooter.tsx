...
function TestFooter({
    textColorClass, settings, elapsedSeconds, mutedTextClass, handleEndTest, formatTime, handleCreateFlashcard, handleFeedback
}: any) {
    ...
    return (
        ...
        <footer style={{ backgroundColor: settings?.themeColor === 'dark' ? '#111828' : settings?.primaryColor }} className={`footer h-14 ${settings.theme === 'dark' ? 'border-t border-gray-600' : ''} flex items-center justify-between px-4 text-xs`}>
            {settings.showTimer && (
                <div className={`block-time flex items-center gap-1 font-semibold text-base ${textColorClass}`}>
