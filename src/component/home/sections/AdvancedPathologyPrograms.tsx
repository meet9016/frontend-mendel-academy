  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-[60vh] bg-white px-4 md:px-6 lg:px-8 py-15">
      ...
      <div className="text-center space-y-1">
        <h1 className="text-4xl md:text-3xl font-extrabold ff-font-bold">Advanced Pathology</h1>
        <h1 className="text-4xl md:text-3xl font-extrabold ff-font-bold">Programs</h1>
      </div>
      <div className="text-center mt-4 ff-font text-base md:text-lg">
        <p>Specialized training designed by pathology experts to advance your diagnostic</p>
        <p>expertise and clinical knowledge.</p>
      </div>

      {/* ----  FEATURED LIVE  ---- */}
      <FeaturedLive onMore={() => router.push("/subscription")} />

      {/* ----  RECORDED  ---- */}
      <Section title="Recorded Programs" subtitle="Self-paced learning with lifetime access">
        {loading ? <ProgramSkeleton /> : <RecordedGrid programs={programs} onCart={addToCart} />}
      </Section>

      {/* ----  UPCOMING  ---- */}
      <Section title="Upcoming Programs" subtitle="Join the waitlist and get early access">
...
/* ----------  SUB-COMPONENTS  ---------- */
const FeaturedLive = ({ onMore }: { onMore: () => void }) => (
  <div className="w-full max-w-[1025px] mx-auto bg-gray-900 text-white rounded-2xl p-6 md:p-10 mt-10 shadow-lg hover:shadow-xl transition-shadow">
    ...
    <h3 className="text-2xl md:text-3xl ff-font-bold font-bold mb-4">Molecular Pathology Training</h3>
    ...
    <div className="flex items-center gap-3 mb-5">
      ...
      <div>
        <h4 className="font-semibold ff-font-bold text-white text-base md:text-lg">Dr. John Doe</h4>
        <p className="text-gray-400 ff-font text-sm">MD, Senior Pathologist</p>
      </div>
    </div>

    <div className="flex flex-wrap gap-2 mb-6">
      {["Hands-on Practice", "Case Discussions", "Certification"].map((f) => (
        <div key={f} className="bg-gray-700 text-white ff-font px-4 py-1.5 rounded-full text-sm hover:bg-gray-600 transition">
          {f}
        </div>
      ))}
    </div>

    <CommonButton size="xxl" pyClass="py-4" pxClass="px-10" fontWeight={700} fontSize={14} onClick={onMore} className="group">
...
const RecordedGrid = ({ programs, onCart }: { programs: Program[]; onCart: (p: Program) => void }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 cursor-pointer">
    {programs.map((p) => (
      <div key={p.id} className="group bg-white border border-gray-200 rounded-xl overflow-hidden shadow hover:shadow-lg hover:-translate-y-1 transition">
        ...
        <div className="p-4">
          <h3
            ...
            style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", lineHeight: "1.4rem", height: "2.8rem" }}
          >
            {p.title}
          </h3>
          <p
            className="text-xs ff-font mb-3 overflow-hidden text-ellipsis line-clamp-2"
            style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", lineHeight: "1.2rem", height: "2.4rem" }}
          >
            {p.subtitle}
          </p>
          ...
          <div className="flex items-end justify-between">
            ...
            <CommonButton pyClass="py-0" pxClass="px-2" fontWeight={700} fontSize={14} onClick={() => onCart(p)}>
              Add to cart
            </CommonButton>
          </div>
        </div>
...
const UpcomingGrid = ({ program }: { program: Record<string, any> }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 cursor-pointer">
    {[...Array(3)].map((_, i) => (
      <div key={i} className="group from-gray-100 to-white border border-gray-200 rounded-xl p-4 shadow hover:shadow-md hover:-translate-y-1 transition relative">
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-300 rounded-xl" />
        <div className="flex items-center justify-between mb-2">
          <div className="text-[10px] font-bold px-2 py-1 bg-white text-primary ff-font rounded-md uppercase">LAUNCHING SOON</div>
        ...
        <div className="bg-gray-100 rounded-md p-2 mb-3">
          ...
          <div className="w-full bg-gray-200 h-1.5 rounded-full mt-1">
            <div className="bg-[#FFCA00] h-1.5 rounded-full" style={{ width: `${program.progress}%` }} />
          </div>
        </div>
        <div className="flex items-center gap-2 bg-gray-100 rounded-md p-2 mb-3">
          <FaClock className="text-primary" />
          <div>
            <p className="text-xs font-semibold ff-font">Starts in {program.startDays} days</p>
            <p className="text-[10px] ff-font">{program.duration}</p>
          </div>
        </div>
        <p className="text-xs ff-font-bold mb-4">{program.description}</p>
        <CommonButton pyClass="py-0" pxClass="px-16" fontWeight={600} fontSize={13} className="ff-font-bold">
          Join Waitlist
        </CommonButton>
        <p className="text-[11px] ff-font text-center mt-2">No payment now · Email reminder before launch</p>
      </div>
    ))}
  </div>
);
