...
//     // Check if there is an active test session to resume
...
//         if (!userId) {
//           const proFlag = window.localStorage.getItem("mendel_qbank_pro") === "1";
//           setIsPro(proFlag);
//           return;
//         }
...
//         if (response?.data) {
//           const activePlan = response.data.cart?.payBill?.find(
//             (p: any) => p.cart_type === "qbank_plan"
//           );
//           if (activePlan) {
//             setIsPro(true);
//             window.localStorage.setItem("mendel_qbank_pro", "1");
//           } else {
//             setIsPro(false);
//             window.localStorage.setItem("mendel_qbank_pro", "0");
//           }
//         }
//       } catch (error) {
...
//         const selected = allDemo.slice(0, 10);
...
//         try {
//           const resAttempt = await api.post(endPointApi.createTestAttempt as string, {
//             mode,
...
//         const id = parsed.attemptId || parsed.id;
//         if (id) {
//           router.push(`/test-run?id=${id}`);
//           return;
//         }
//       } catch (e) { }
//     }
...
//             {!isPro && (
//               <div className="mb-4 rounded-md border border-yellow-300 bg-yellow-50 px-4 py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
//                 <div>
//                   <p className="text-sm font-semibold text-yellow-800">You are in demo version</p>
//                   <p className="text-xs text-yellow-700">
//                     Unlock the Pro version to get full access to all questions and advanced features.
//                   </p>
//                 </div>
//                 <button
//                   type="button"
//                   onClick={() => router.push("/qbank-plans")}
//                   className="px-4 py-2 bg-primary cursor-pointer text-dark rounded text-sm font-medium hover:bg-primary/80"
//                 >
//                   Unlock Pro Version
//                 </button>
//               </div>
...
//             {canResume && (
//               <div className="mb-4 rounded-md border border-blue-200 bg-blue-50 px-4 py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
...
//                     className={`relative inline-flex h-6 w-11 items-center rounded-full cursor-pointer ${tutorEnabled ? "bg-primary border-gray-700" : "bg-gray-200 border-gray-300"
//                       }`}
//                   >
//                     <span
...
//                     className={`relative inline-flex h-6 w-11 items-center rounded-full cursor-pointer ${timedEnabled ? "bg-primary border-gray-700" : "bg-gray-200 border-gray-300"
//                       }`}
//                   >
//                     <span
...
//                           {subject.name}<CountBadge count={subject.questionCount} color="gray" />
//                         </span>
//                       </label>
//                     ))}
//                   </div>
...
//                         <div
//                           key={chapter.id}
//                           className={`rounded px-2 py-2 text-base bg-white transition-all ${disabled ? "opacity-50" : "hover:border-gray-300"
//                             }`}
//                         >
//                           {/* Chapter row */}
...
//                             />
//                             <span
//                               className={`flex-1 min-w-0 truncate ${disabled ? "text-gray-400" : "cursor-pointer hover:text-gray-600"
//                                 }`}
...
//                   Available Questions: <span className="font-semibold text-gray-700">{availableQuestionCount}</span>
//                 </p>
...
//                     className="w-28 px-3 py-2 border rounded text-base focus:ring-1 focus:ring-gray-500 focus:border-gray-500"
//                   />
...
//                 className="px-7 py-2.5 bg-primary cursor-pointer text-dark rounded text-base hover:bg-primary/80 disabled:opacity-50 transition-colors shadow-sm hover:shadow"
//               >
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="text-base font-semibold text-gray-800 flex items-center gap-3">
          ...
          {badgeCount !== undefined && (
            <span className={`shrink-0 text-sm font-medium border ml-1 rounded-full px-2 py-0.5 text-gray-700 border-gray-200`}>
              {badgeCount}
            </span>
          )}
        </div>
...
export default function TestCreatePage() {
  ...
  // ─── Check pro status and resume capability ───────────────────────────────
  useEffect(() => {
    if (typeof window === "undefined") return;
  ...
  const handleToggleAllSubjects = useCallback(() => {
    if (allSubjectsSelected) {
      setSelectedSubjectIds([]);
      setSelectedChapterIds([]);
      setSelectedTopicIds([]);
  ...
  }, [allSubjects, allSubjectsSelected]);
  ...
  }, [allChaptersSelected, selectableChapterIds, selectedChapterIds, allTopics]);
  ...
  const handleToggleChapter = useCallback((chapter: Chapter) => {
    if (!selectedSubjectIds.includes(chapter.subjectId)) return;
  ...
  const handleToggleTimed = useCallback(() => {
    setTimedEnabled((prev) => {
      const next = !prev;
      setMode(next ? "timed" : tutorEnabled ? "tutor" : "tutor");
      return next;
  ...
  const handleQuestionCountBlur = useCallback(() => {
    if (localQuestionCount === "" || parseInt(localQuestionCount, 10) === 0) {
      setLocalQuestionCount("1");
      setQuestionCount(1);
      return;
    }
  ...
  }, [localQuestionCount, availableQuestionCount]);
        ...
        <section className="flex-1 bg-[#f3f3f4]">
          ...
          <div className="px-4 md:px-8 py-4 space-y-5 md:space-y-0">
            ...
            <Accordion 
              ...
              onToggle={handleAccordionToggle}
              badgeCount={tree?.totalQuestions ?? 0}
            >
              <div className="px-4 py-3 flex items-center gap-4">
                <div className="flex text-sm border border-gray-300 rounded-full overflow-hidden">
                  <button className="px-4 py-1.5 bg-white text-gray-700 border-r border-gray-300">
            ...
            <Accordion
              title="Subjects"
              id="subjects"
              isOpen={accordionStates.subjects}
              onToggle={handleAccordionToggle}
              showSelectAll
              selectAllChecked={allSubjectsSelected}
              onSelectAll={handleToggleAllSubjects}
            ...
            <Accordion
              title="Chapters"
              id="chapters"
              isOpen={accordionStates.chapters}
              onToggle={handleAccordionToggle}
              showSelectAll
              selectAllChecked={allChaptersSelected}
              onSelectAll={handleToggleAllChapters}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {chapter.name}
                              <span className="shrink-0 text-sm font-medium border ml-1 rounded-full px-2 py-0.5 text-gray-700 border-gray-200">
                                {disabled ? 0 : chapter.questionCount}
                              </span>
                            </span>
